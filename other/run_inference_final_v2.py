...
def ensemble_predictions(all_preds, nms_threshold=0.47, boost_factor=0.03, penalty_factor=0.85, conf_cutoff=0.25, max_boost=0.12):
    """对所有模型预测出来的框进行融合与 NMS 去重，并应用基于多模型共识的置信度奖励与惩罚机制"""
    if not all_preds:
        return []
...
def main():
    ...
    # 4. 推理主循环
    print("开始融合通道并执行 5 折模型集成推理（Vote Boosting + AABB-NMS）...")
