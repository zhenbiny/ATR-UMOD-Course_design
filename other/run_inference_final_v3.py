...
# TEST_RGB_DIR = r"E:\TEST\images"
# TEST_IR_DIR = r"E:\TEST\images_ir"

TEST_RGB_DIR = r"E:\images"
TEST_IR_DIR = r"E:\images_ir"
# ==============================================================================
...
def ensemble_predictions(all_preds, nms_threshold=0.47, min_agreement=2, conf_cutoff=0.30):
    """
    对所有模型预测出来的框进行融合与 NMS 去重，并应用“多模型共识筛选机制”：
    只有被至少 min_agreement 个模型同时检测到的目标才予以保留，孤立检测直接丢弃。
    """
    if not all_preds:
        return []
    ...
    for class_idx in range(len(CLASSES)):
        ...
        for idx, overlap_cnt in zip(keep, counts):
            conf = cls_scores[idx]
            # 严格共识制过滤：只有被多折模型共同验证检测到的框才放行，孤立虚警框直接丢弃
...
def main():
    ...
    # 4. 推理主循环
    print("开始融合通道并执行 5 折模型集成推理（两模型严格共识降噪 + AABB-NMS）...")
    for filename in tqdm(test_files):
        ...
        # 执行跨模型预测融合去重与严格的两模型共识机制筛选
        fused_predictions = ensemble_predictions(
            raw_predictions, 
            nms_threshold=0.47, 
            min_agreement=2, 
            conf_cutoff=0.30
