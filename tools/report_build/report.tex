...
\subsection{测试集评测结果与泛化分析}
在不公开测试数据集上，本算法模型取得了 **0.5504 (55.04\%)** 的 mAP@50 成绩。

本地验证集与测试集表现存在由于评测集分布不同而造成的泛化衰减。本地验证集虽然保证了地理位置不重合，但测试集中包含更为极端的突发域变化（如光照骤变、极端天气及遮挡场景等），在未见环境的测试下，算法容易出现精度衰减。在细分品类中，特别是在卡车和挂车两个长条形大目标上，旋转框对大纵横比细长目标的边界提取效果更为贴合。
